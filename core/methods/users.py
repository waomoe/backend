        ...
        async def getUser(
            ...
            if len(errors) == 0:
                ...
                if len(errors) == 0:
                    ...
                    is_friend = (
                        (
                            target.user_id in user.following
                            and user.user_id in target.following
                        )
                        if (user and user.following)
                        else False
                    )
                    is_blocked = (
                        (user.user_id in target.blocked_users)
                        if (user and target.blocked_users)
                        else False
                    )
                    target_privacy = await target.get_privacy_settings()
                    for key in target_privacy.keys():
                        if (
                            is_me or (is_friend and target_privacy[key] == "friends")
                        ) and not is_blocked:
                            keys += [key]
                    target.privacy
